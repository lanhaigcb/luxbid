...
public class DefaultRestErrorResolver implements RestErrorResolver, MessageSourceAware, InitializingBean {
    ...
    protected final Map<String, String> createDefaultExceptionMappingDefinitions() {
        ...
        // 400
        applyDef(m, HttpMessageNotReadableException.class, CommonResponseCode.COMMON_ILLEGAL_PARAM);
        applyDef(m, MissingServletRequestParameterException.class, CommonResponseCode.COMMON_ILLEGAL_PARAM);
        applyDef(m, TypeMismatchException.class, CommonResponseCode.COMMON_ILLEGAL_PARAM);
        applyDef(m, "javax.validation.ValidationException", CommonResponseCode.COMMON_ILLEGAL_PARAM);

        // 404
        applyDef(m, NoSuchRequestHandlingMethodException.class, CommonResponseCode.COMMON_RESOURCE_NOT_FOUND);
        applyDef(m, "org.hibernate.ObjectNotFoundException", CommonResponseCode.COMMON_RESOURCE_NOT_FOUND);

        // 405
        applyDef(m, HttpRequestMethodNotSupportedException.class, CommonResponseCode.COMMON_REQUEST_METHOD_NOT_SUPPORT);

        // 406
        applyDef(m, HttpMediaTypeNotAcceptableException.class, CommonResponseCode.COMMON_REQUEST_MEDIA_TYPE_NOT_SUPPORT);

        // 409
        //can't use the class directly here as it may not be an available dependency:
        applyDef(m, "org.springframework.dao.DataIntegrityViolationException", CommonResponseCode.COMMON_SERVER_ERROR);

        // 415
        applyDef(m, HttpMediaTypeNotSupportedException.class, CommonResponseCode.COMMON_REQUEST_MEDIA_TYPE_NOT_SUPPORT);

        return m;
    }

    private void applyDef(Map<String, String> m, Class clazz, CommonResponseCode status) {
        applyDef(m, clazz.getName(), status);
    }

    private void applyDef(Map<String, String> m, String key, CommonResponseCode status) {
        m.put(key, definitionFor(status));
    }

    private String definitionFor(CommonResponseCode status) {
        return status.code() + ", " + DEFAULT_EXCEPTION_MESSAGE_VALUE;
    }
    ...
    public RestError resolveError(ServletWebRequest request, Object handler, Exception ex) {
        ...
        builder.setResponseCode(getStatusValue(template, request, ex));
    ...
    private RestError getRestErrorTemplate(Exception ex) {
        ...
        if (ex instanceof BaseException) {
            String exceptionInfo = ex.getLocalizedMessage();
            template = new RestError(((BaseException) ex).getCommonResponseCode(), "_exmsg", exceptionInfo, null, ex);
            return template;
        }
