...
@RequestMapping("/c2cSellOrder")
public class C2CSellOrderController extends BaseController {
    @Autowired
    ...
    public ModelAndView index() {
        ModelAndView mv = new ModelAndView("c2cSellOrder/index");
    ...
    public Object list(Integer customerId, String account, Integer startMinute,
        ...
        if (Strings.isNullOrEmpty(status)){
            status = "EX_ORDER_STATUS_TRADE";
        }
        if (Strings.isNullOrEmpty(type)){
            type = "EX_ORDER_TYPE_SELL";
        }
